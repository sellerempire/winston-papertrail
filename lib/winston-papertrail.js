...
Papertrail.prototype.log = function (level, msg, meta, callback) {
	...
	/*** ⬇️⬇️⬇️ CHARLIE REFACTORED SOURCE CODE ⬇️⬇️⬇️ ***/

	/*** new object so symbols can be taken off ***/
	/*** NOTE: Duplicate data was being added. ***/
	let newObject = {};

	/*** add everything but 'Symbols' to object ***/
	for (const [key, value] of Object.entries(meta)) {

		/*** add key and value to newObject ***/
		newObject[key] = value;
	}

	/*** meta equals newObject without 'Symbols' ***/
	meta = newObject;

	/*** ⬆️️️️️⬆️️️️️⬆️️️️️ CHARLIE REFACTORED SOURCE CODE ⬆️️️️️⬆️️️️️⬆️️️️️️ ***/
	...
	/*** add level and message to output ***/
	var output = 'Level: ' + level.toUpperCase() + ',\n' + 'Message: ' + msg + ',';
	...
	if (meta) {
		if (typeof meta !== 'object') {
			...
			if (this.inlineMeta) {
				...
				/*** Commenting out because the util.inspect is not consistent in formatting data ***/
	...
	/*** iterate through split error stack ***/
	for (const [index, value] of outputSplit.entries()) {

		/*** trim white space, add new line ***/
		outputSplit[index] = outputSplit[index].trim() + '\n';

		/*** pad error message with 4 spaces ***/
		if (index > 12) {
			outputSplit[index] = '    ' + outputSplit[index];
		}
	}

	/*** join array and create string ***/
...
Papertrail.prototype.sendMessage = function (hostname, program, level, message, callback) {
	...
	for (var i = 0; i < lines.length; i++) {
		...
		/*** add gap after second line, not first ***/
		if (i == 2) {
			gap = '    ';
		}

		/*** NOTE: When Winston was upgraded to version 3.0 it created ***/
		/*** a number of breaking changes for winston-papertrail. The ***/
		/*** original maintainers have not upgraded winston-papertrail ***/
		/*** to work with Winston 3.0. One of them is below on line 456, ***/
		/*** winston.config.colorize(level). I am adding color to the papertrail ***/
		/*** logs in the logformat() via escape sequences where you can format ***/
		/*** your message. Down the road if we needed to add it via this source code ***/
		/*** it would need to be done via the winston.format.colorize() created in v3.0 ***/
